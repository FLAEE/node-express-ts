...
app.get('/api/list/:id', (req: Request, res: Response)=>{
    const course = courses.find(c => c.id === parseInt(req.params.id));
    if (!course) return res.status(404).send(`Entry with ID ${req.params.id} does not exist`)
    res.send(course);
});
...
app.put('/api/list/:id', (req : Request, res : Response)=> {
    const course = courses.find(c => c.id === parseInt(req.params.id));
    if (!course) return res.status(404).send(`Entry with ID ${req.params.id} does not exist`)
